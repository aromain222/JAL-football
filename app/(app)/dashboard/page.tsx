...
export default async function DashboardPage() {
  ...
  const activityFeed = [
    ...recentReviews.map((review) => ({
      ...
      detail: review.note ?? "Review logged.",
    ...
    ...recentShortlisted.map((item) => ({
      ...
      label: item.stage,
      detail: item.player
        ? `${item.player.first_name} ${item.player.last_name} → ${item.stage.replace("_", " ")}`
        : `Player → ${item.stage.replace("_", " ")}`,
      meta: item.need?.title ?? "Shortlist"
    }))
  ]
    .sort((a, b) => b.created_at.localeCompare(a.created_at))
    .slice(0, 8);
      ...
      <div className="rounded-2xl border border-[#e4e8e5] bg-white p-5">
        ...
        <div className="mt-3 flex items-center gap-2">
          ...
          <WorkflowStep
            ...
            label="Shortlist"
            href="/shortlist"
            count={metrics.shortlistedPlayers}
            countLabel="players"
          />
        </div>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
        ...
        <section className="rounded-2xl border border-[#e4e8e5] bg-white">
          <div className="flex items-center justify-between border-b border-[#e4e8e5] px-5 py-4">
            ...
            <Link href="/needs" className="text-[12px] text-[#4b5563] hover:text-[#111827]">
              All needs →
            </Link>
          </div>
          <div className="grid gap-2 p-4">
            {needs.length ? needs.map((need) => (
              <div
                key={need.id}
                className={cn(
                  "flex items-center justify-between gap-3 rounded-xl border border-[#e4e8e5] bg-[#f8f9fa] px-4 py-3",
                  need.priority === "critical" && "border-l-2 border-l-red-400"
                )}
              >
                <div className="min-w-0">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-[#9ca3af]">
                    {need.position} · {need.priority === "critical" ? "Critical" : "Active"}
                  </p>
                  <p className="mt-0.5 truncate text-[14px] font-medium text-[#111827]">{need.title}</p>
                </div>
                <Link
                  href={`/review/${need.id}`}
                  className="flex shrink-0 items-center gap-1.5 rounded-xl bg-[#15542a] px-3 py-1.5 text-[12px] font-medium text-white hover:bg-[#1a6934]"
                >
                  Review
              ...
              <div className="rounded-xl border border-dashed border-[#e4e8e5] p-6 text-center text-sm text-[#9ca3af]">
                No active needs.{" "}
                <Link href="/needs/new" className="text-[#15542a] underline">Create one →</Link>
              </div>
            )}
          </div>
        </section>
            ...
            {activityFeed.length ? activityFeed.map((item) => (
              <div key={item.id} className="flex items-start gap-3 px-5 py-3">
                <div className={cn(
                  ...
                  item.type === "shortlist" ? "bg-blue-400" :
                  item.label === "right" ? "bg-emerald-400" : "bg-[#c8d0cb]"
                )} />
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 flex items-center gap-1.5 text-[11px] text-[#9ca3af]">
                    ...
                    {formatDate(item.created_at)} · {item.meta}
                  </p>
                </div>
              </div>
            )) : (
            ...
            {recentShortlisted.map((item) => (
              <div key={item.id} className="rounded-xl border border-[#e4e8e5] bg-[#f8f9fa] px-4 py-3">
                ...
                <p className="mt-1 text-[11px] text-[#9ca3af]">
                  {item.need?.title ?? "Shortlist"} · {formatDate(item.updated_at ?? item.created_at)}
                </p>
              </div>
            ))}
          </div>
...
function WorkflowStep({
  ...
  return (
    <Link
      href={href}
      className="flex flex-1 flex-col gap-1 rounded-xl border border-[#e4e8e5] bg-[#f8f9fa] px-4 py-3 transition-colors hover:border-[#c8d0cb] hover:bg-white"
    >
      ...
      <span className="text-[12px] font-mono text-[#15542a]">
        {count} {countLabel}
      </span>
    </Link>
  );
