...
const conferences = ["ALL","AAC","ACC","Big 12","CUSA","MAC","Mountain West","SEC","Sun Belt"];
      ...
      <CardContent className="p-0">
        <div className="relative overflow-hidden border-b border-[#d6ddd8] bg-[linear-gradient(135deg,#10251e_0%,#183327_55%,#204234_100%)] px-5 py-5 text-white">
          ...
          <div className="relative flex items-start gap-3">
            ...
            <div>
              ...
              <h2 className={`${scoutingDisplay.className} mt-2 text-[1.9rem] uppercase leading-none tracking-[0.04em] text-[#f4efe2]`}>Refine the Pool</h2>
              <p className="mt-2 max-w-xl text-sm leading-6 text-[#d8e1d5]/75">Work from broad scope to tight scope. Lock in hard requirements first, then narrow by measurables.</p>
            </div>
          </div>
        </div>
        <form className="grid gap-4 p-5 lg:grid-cols-4">
          ...
          <div className="relative lg:col-span-2">
            ...
            <Input className="h-11 rounded-[18px] border-[#cfd8d2] bg-white/90 pl-9" defaultValue={defaults.search} name="search" placeholder="Search player, school, or tag signal" />
          </div>
          <FilterGroup label="Position">
            <select className="h-11 rounded-[18px] border border-[#cfd8d2] bg-white/90 px-3 text-sm" defaultValue={defaults.position ?? "ALL"} name="position">
              {positions.map((o) => <option key={o} value={o}>{o === "ALL" ? "All positions" : o}</option>)}
            </select>
          </FilterGroup>
          <FilterGroup label="Class Year">
            <select className="h-11 rounded-[18px] border border-[#cfd8d2] bg-white/90 px-3 text-sm" defaultValue={defaults.classYear ?? "ALL"} name="classYear">
              {classYears.map((o) => <option key={o} value={o}>{o === "ALL" ? "All classes" : o}</option>)}
            </select>
          </FilterGroup>
          ...
          <FilterGroup label="Years Remaining">
            <select className="h-11 rounded-[18px] border border-[#cfd8d2] bg-white/90 px-3 text-sm" defaultValue={defaults.yearsRemaining ?? "ALL"} name="yearsRemaining">
              {yearsRemainingOptions.map((o) => <option key={o} value={o}>{o === "ALL" ? "Any years left" : `${o} year${o === "1" ? "" : "s"}`}</option>)}
            </select>
          </FilterGroup>
          ...
          <FilterGroup label="Conference">
            <select className="h-11 rounded-[18px] border border-[#cfd8d2] bg-white/90 px-3 text-sm" defaultValue={defaults.conference ?? "ALL"} name="conference">
              {conferences.map((o) => <option key={o} value={o}>{o === "ALL" ? "All conferences" : o}</option>)}
            </select>
          </FilterGroup>
          ...
          <div className="flex items-end gap-3 pt-2 lg:col-span-4">
            <Button className="bg-[#163627] text-[#ebf4ee] hover:bg-[#1b4330]" type="submit">Apply field</Button>
